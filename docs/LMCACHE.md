...
**Do not use the nightly pairing** (`latest-nightly-cu129`, lmcache 0.5.2-dev releases through at least 2026-07-06): it rejects this model's KV layout with `Unsupported EngineKVFormat: 10` on **every store** — and because the error is logged by the sidecar while serving continues, the profile silently degrades to vLLM's in-GPU prefix cache only; every "tiered revisit" you measure is a mirage. Our first fix attempt — a hand-written format-10 transfer kernel ([patch, now **withdrawn**](../patches/README.md#lmcache-format-10-kernel-patch-separate-project)) — made stores *run* but **corrupted the GDN recurrent state on round-trip** (needle vanishes after reload; tool-eval 88 → 47). The correct fix is LMCache **`main`** (≥ `0.5.2.dev66`), which implements `NL_X_NB_NH_BS_TWO_HS` natively — spec, format detector, CUDA kernels, tests, and opaque GDN state pages. Whatever you run: **needle-test across a restart before trusting any external KV tier on a hybrid model** — hit counters and coherent output do not prove fidelity.
