...
The HF quantized drafters (`syvai/Qwen3.8-27B-DFlash2-W4A16` 1.2 GB, `lued/Qwen3.8-27B-DFlash2-W8` 2.0 GB) failed to load on upstream #52816 with `'QKVParallelLinear' object has no attribute 'weight'`. Upstream already threads `get_draft_quant_config()` into the drafter's layers; the one break is `DFlashQwen3Model._build_context_kv_buffers`, which reads `qkv_proj.weight[q_size:]` densely to precompute context KV in a single fused GEMM — on a compressed-tensors pack-quantized layer that is `weight_packed` + `weight_scale`. `patches-dflash2/0002-quantized-drafter-kv-precompute.diff` ports syv-ai's `_dense_kv_rows` ([qwen38-27b-rtx3090](https://github.com/syv-ai/qwen38-27b-rtx3090) `patches/dflash2-backport.patch`): the buffers are built at the end of `load_weights`, before `process_weights_after_loading`'s Marlin repack, so the packed tensors are still in checkpoint layout and dequantize there (symmetric, group strategy). The backport's quantized-lm_head hunk is not needed here (our targets keep `lm_head` unquantized). `test_dense_kv_rows.py` is a CPU round-trip that runs inside the image and is bit-exact for W4 and W8. Reference on the same profile with the bf16 drafter, gittensor target, fp8 KV, plain, max-len 100K (131K refused), 2026-08-22: pool 108,849 · c1 203.8 t/s (peak 251) · deep-30K 177 · c4 297 aggregate / 154 per stream · pp8192 14.1K · needle 40K hit cold+warm · tool-eval 69×2 89. Measured 2026-08-22 (`results/2026-08-22-dflash2-quant-drafters/`, same profile, T=0.6):

| drafter | size | max-len | KV pool | decode c1 / 30K-deep / c4 aggregate | acceptance per draft token | tool-eval 69×2 |
|---|---|---|---|---|---|---|
| incoai bf16 | 3.85 GB | 100K (131K refused) | 108,849 | 204 / 177 / 297 | 0.39 | 89 |
| [syvai W4A16](https://huggingface.co/syvai/Qwen3.8-27B-DFlash2-W4A16) | 1.2 GB | **131K** | **164,550** | 179 / 210 / 463 | 0.39 | **90.5 ± 2.1** |
| [lued W8](https://huggingface.co/lued/Qwen3.8-27B-DFlash2-W8) | 2.0 GB | 131K | 150,022 | 201 / 177 / 380 | 0.37 | 88 ± 1.4 |

The W4A16 drafter buys +56K tokens of pool and a 131K ceiling at no acceptance or tool-eval cost (the single-run c4 column is noisy; pool, ceiling and single-stream are the robust signals). It does not change the daily verdict — 42% of the daily's pool, no tiers, no NVFP4 KV, +5% single-stream over MTP on the same runner — but it is the best ≤131K single-user profile on this card: `IMAGE=vllm-qwen38:tiers-nvfp4kv-dflash2 KV=fp8_e4m3 DRAFT_DIR=<syvai W4A16> MAXLEN=131072 NS=7 scripts/serve-dflash2.sh`.
