...
set -euo pipefail
cd "$(dirname "$0")/.."
for id in 0142c 0142b; do
    scratch=".work/$id"
    rm -rf "$scratch"
    mkdir -p "$scratch"
    cp -R src/vllm "$scratch/vllm"
    if [[ "$id" == 0142c ]]; then slug=dflash-sync-census; else slug=dflash-device-side-accept; fi
...
patch -d .work/0142c -p1 --fuzz=0 --batch --forward --dry-run < deliver/0142b-dflash-device-side-accept-v0290.diff
patch -d .work/0142c -p1 --fuzz=0 --batch --forward < deliver/0142b-dflash-device-side-accept-v0290.diff || { status=$?; exit "$status"; }
python3 -m py_compile .work/0142c/vllm/envs.py .work/0142c/vllm/v1/worker/gpu/model_runner.py
python3 deliver/test-0142.py .work/0142c
