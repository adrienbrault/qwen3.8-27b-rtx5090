...
**Official SWE-Bench-Verified score: 67.0%** (100-task random subset, patches replayed through the official `swebench` harness; 4.2 h at c4 on this box, 84.6% external prefix hit). R2E-Gym's own reward said 70/100 and every officially-resolved task was also R2E-solved — so the harness's reward signal runs ~3 pts optimistic, one-directional; the sweep/A/B tables above use it for *relative* comparisons only. For calibration against other Qwen3.6-27B results: ~67.8% is the published same-model mini-swe-agent reference, 79.2% the public SOTA, 88–90% only with heavily engineered claude-cli agent stacks — i.e. a stock scaffold on one 5090 lands right at the reference band, and the remaining headroom is agent-scaffold engineering, not engine configuration.
