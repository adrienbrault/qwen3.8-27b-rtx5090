...
The served configuration since 2026-09-09: two RTX 5090, [nvidia/Qwen3.8-27B-NVFP4](https://huggingface.co/nvidia/Qwen3.8-27B-NVFP4) weights (the [RedHatAI](https://huggingface.co/RedHatAI/Qwen3.8-27B-NVFP4) checkpoint served from 2026-09-02 to 2026-09-09, [R231](bench/results/r231-promote-nvidia.md)), [vLLM](https://github.com/vllm-project/vllm) 0.29 with an NVFP4 KV cache pinned at 14.86 GB per GPU for a 1,391,795-token pool ([R234](bench/results/r231-promote-nvidia.md)), the checkpoint's own MTP head at 3 draft tokens (the DFlash2 drafter served until 2026-09-06, [R207](bench/results/r207-promote-mtp.md)), FlashInfer's `pcie_ipc` all-reduce as the two-card decode all-reduce (patch 0138) and vLLM's batch-sharded sampling (each card samples half the batch from local logits; patch 0147 keeps one compile artifact for both settings; R195), launcher [scripts/serve-r231-nvidia-daily.sh](scripts/serve-r231-nvidia-daily.sh). Each row links to its write-up in [bench/RESULTS.md](bench/RESULTS.md), which names the raw results directory on the serving host, and to the driver script that produced it.

| | value | measured |
|---|---|---|
| context length | 262,144 tokens | |
| KV pool on the GPUs | 1,391,795 tokens, pinned at 14.86 GB per card, 16 sequences | 2026-09-09, [R234](bench/results/r231-promote-nvidia.md). Was 1,309,368 at 13.98 GB, 2026-09-06, [R207](bench/results/r207-promote-mtp.md); 1,052,277 on the DFlash2 drafter route, 2026-09-05, [R197](bench/results/r197-spec-length-ladder.md). The pool follows the pin, not the weights. Higher pins were not limited by memory but by a warmup-flake rate that rises with the pin: 15.90 GB booted 2 of 3 and 16.90 GB 1 of 3, both still leaving over 1,371 MiB free under load |
...
| decode, 1 stream | • code 216 t/s<br>• prose 164 t/s<br>• prose at 30K context 158 t/s | 2026-09-09, [R234](bench/results/r231-promote-nvidia.md) on the NVIDIA checkpoint at the 14.86 GB pin; the RedHat checkpoint read 227 / 170 / 157 on the same instrument at the 13.98 GB pin the same day, and 209 / 155 / 149 on 2026-09-06, [R207](bench/results/r207-promote-mtp.md) |
| decode, 8 streams | • code 1,476 t/s aggregate, 185 per stream<br>• prose 1,267 t/s aggregate, 158 per stream | 2026-09-09, [R234](bench/results/r231-promote-nvidia.md); RedHat read 1,482 / 1,250 the same day and 1,539 / 1,231 on 2026-09-06, [R207](bench/results/r207-promote-mtp.md) |
| decode, 16 streams | • code 2,596 t/s aggregate, 162 per stream<br>• prose 2,183 t/s aggregate, 136 per stream | 2026-09-09, [R234](bench/results/r231-promote-nvidia.md); RedHat read 2,594 / 2,174 the same day and 2,634 / 2,145 on 2026-09-06, [R207](bench/results/r207-promote-mtp.md) |
...
| [tool-eval](https://github.com/SeraphimSerapis/tool-eval-bench), 69 scenarios × 4 trials | 88.5 ± 0.6 and 90.5 ± 3.7, two runs | 2026-09-09 on the NVIDIA checkpoint, [R231/R234](bench/results/r231-promote-nvidia.md). The RedHat checkpoint read 91.2 ± 0.5 on the same instrument, 2026-09-06, [R207](bench/results/r207-promote-mtp.md). The second interval contains both the first reading and RedHat's, and the per-trial scores were [123, 122, 123, 122] on one run against [127, 131, 122, 120] on the other, so a single run of this benchmark does not establish a gap of this size on this stack |
...
| fidelity vs the [bf16 model](https://huggingface.co/Qwen/Qwen3.8-27B), dense text, 555,549 positions | • top-1 agreement 90.67%<br>• perplexity +1.83%<br>• truncated KL 0.0226 | 2026-09-09 on the NVIDIA checkpoint, [R231](bench/results/r231-promote-nvidia.md). The RedHat checkpoint it replaces read 92.79% / +0.83% / 0.0143 on the same ruler; two boots of one configuration differ by 0.10 to 0.15% on perplexity, so the gap is about seven times the noise floor. The checkpoint was promoted with that number in hand, on the other measured quantities. The bf16 *decode* ruler disagrees and is recorded in the write-up. [docs/FIDELITY.md](docs/FIDELITY.md) |
| fidelity vs the bf16 model, agentic turns, 57,972 positions | • top-1 agreement 95.63%<br>• perplexity +2.67% | **Not re-measured on the served checkpoint.** These are the RedHat figures, 2026-09-06 on the MTP route, [R206](bench/results/r206-mtp-vs-dflash-paired.md); the NVIDIA checkpoint has only been measured on the dense ruler. [docs/FIDELITY.md](docs/FIDELITY.md) |
