# Qwen3.6-27B on a single RTX 5090 — 253K KV pool, fp8 cache, MTP K=4 speculative decoding
...
- **253K-token KV pool → 200K usable context** on `fp8_e4m3` KV — the fp8 attention path is flat with depth (no decode crater) where custom 4-bit-KV kernels crater.
...
- `--mamba-cache-mode align --enable-prefix-caching --enable-chunked-prefill` — hybrid-model prefix caching; **`align` is load-bearing for the 253K pool** (it packs the GDN/Mamba state into the unified KV pool). Mode `all` is same speed / same pool and does **not** avoid the crash — don't bother.
- `--gpu-memory-utilization 0.94 --max-model-len 200000` — let vLLM profile the pool; don't hand-set `--kv-cache-memory` (its "fully utilize" hint ignores warmup transients and OOMs).
...
**Verify container identity after launch.** Confirm the startup log reports a **~253K-token KV pool**. A wrong pool size means a preset/dtype/align mismatch reading as a perfectly healthy server at the *wrong* config.
...
3. **Verify the ~253K KV pool in the launch log.** A silent config fallback (wrong preset, `align` off, wrong image) looks like a healthy server at the wrong pool.
...
8. **util 0.95 crashes under concurrent cold starts.** A burst of ~8 simultaneous fresh prompts OOMs the GDN prefill kernel and kills the engine; a ramping benchmark never trips it. Run **0.94**, burst-verified.
...
|---|---|---|---|
| **Lorbus INT4-AutoRound · fp8_e4m3** + FlashInfer + MTP `ns=4` **(current, 2026-07-18)** | INT4-AutoRound · fp8 | **~253K** | Flat deep decode (fp8+FlashInfer has **no** decode crater at depth, where the custom TurboQuant kernel drops); biggest pool yet; **MTP `ns=4`** restored by [PR #42603](https://github.com/vllm-project/vllm/pull/42603); tool-eval 90; and it drops the experimental TurboQuant KV kernel for the **battle-tested fp8** path. Smaller INT4-AutoRound weights free more VRAM for KV than NVFP4, so fp8 KV now reaches 253K (vs ~136K under NVFP4 weights). |
