...
**Agentic benchmarks.** SWE-Bench Verified on the 2026-08-21 daily (saka checkpoint, NVFP4 KV + tiers + V2 runner, R2E-Gym scaffold with its default 40-step soft budget, function calling, T=0.6, 4 parallel tasks, official swebench harness): **331/500 = 66.2%** (one instance still unscored, ceiling 332; run 2026-08-21/22, `results/2026-08-21-deepswe38-nvfp4-tiers`). The Qwen3.6-27B run on the identical scaffold scored 69.4%; most of the gap is that Qwen3.8 obeys the scaffold's "Steps Remaining… submit now" soft budget where 3.6 ignored it (an 80-step A/B is in progress — see [bench/RESULTS.md](bench/RESULTS.md#agentic-benchmarks-on-the-qwen38-daily-2026-08-2122)). Terminal-Bench 2.1 has not been re-run on 3.8 yet; the 3.6 numbers are archived in [bench/RESULTS.md](bench/RESULTS.md#archive--qwen36-era-2026-07) and [docs/HISTORY.md](docs/HISTORY.md). One finding from 2026-08-21 matters for anyone re-running them: R2E-Gym's function-calling gate keys on the served model name; with it off the model never sees the tool schema and scores 5/25 instead of 20/25 on the same tasks (FINDINGS R76).
