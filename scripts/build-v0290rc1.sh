...
# v0.29.0rc1 generation of the Qwen3.8 image chain: the v0280 patch stack REBASED (codex, NOTES16.md — the
# first 'applies unchanged' claim was a broken check); the nvfp4 writer .cu is byte-identical to v0.28.0, so
# 0102 + the overlay stay required.
...
while systemctl is-active --quiet tb21-ladder || systemctl is-active --quiet tb21-ladder2; do sleep 30; done
...
build "$TAG" /srv/qwen5090/patches-v0290 Dockerfile --build-arg OVERLAY_JOBS=4
