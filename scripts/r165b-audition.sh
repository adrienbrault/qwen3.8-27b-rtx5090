#!/usr/bin/env bash
# R165b (2026-09-03): the R165 audition re-run on the rebuilt chain. Changes vs r165-audition.sh: cell A runs on the
# -revival-prs tag (0134 = PR #54163 port; the base tag on rc1 gets 0 prefix-cache hits with DFlash2), the fidelity
# ruler runs at --conc 1 (prompt_logprobs OOM-killed the engine at the default concurrency, FINDINGS R104c), and cell B
# also runs the warm revisit. 0101/0109 fixed for rc1's plain-tensor paged_kv_indices (cell B failed to boot on it).
...
R=/srv/qwen5090/results/2026-09-03-r165b-audition; mkdir -p "$R"
...
log "=== R165b audition start (lock held): $BASE_IMG / $PRS_IMG ==="
...
finish(){ teardown; log "restoring daily"; bash /srv/qwen5090/daily-restore-retry.sh 2>&1 | grep -aE "DAILY|FAILED|KV pool|attempt" | cut -c1-160 | tee -a "$R/audit.log"; log "=== R165b audition $1 ==="; }
...
if boot A-fp8 "$FP8_ENV" "$FP8_SPEC" "$FP8_X" 8; then
  ...
  python3 /srv/qwen5090/probes/fidelity.py run --url $U --model qwen3.8-27b --corpus "$FD/corpus.jsonl" --out "$R/run-v0290rc1-fp8kv.jsonl" --conc 1 > "$R/fidelity-run.log" 2>&1
