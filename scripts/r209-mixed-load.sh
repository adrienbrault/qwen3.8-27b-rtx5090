#!/usr/bin/env bash
# R209 sustained mixed prefill+decode (2026-09-07, user "have you measured perf impact when many requests are prefilling/decoding?
...
#   unit: sudo systemd-run --unit=r209-mixed-load --collect -p User=adrienbrault -p RuntimeMaxSec=21600 -p TimeoutStopSec=300 \
#         -p 'ExecStopPost=-/bin/bash /srv/qwen5090/r208-restore-power.sh' \
#         -E GPU_QUEUE_NAME=r209-mixed-load bash -c '. /srv/qwen5090/lib/gpu-queue.sh; exec bash /srv/qwen5090/r209-mixed-load.sh'
set -uo pipefail
export PATH="$HOME/.local/bin:$PATH"
R=/srv/qwen5090/results/2026-09-07-r209-mixed-load; mkdir -p "$R"
...
M=qwen3.8-27b
BENCHDIR=/srv/qwen5090/native-l2/_r209   # mounted into the container as /l2/_r209; tier-evict only deletes *.tmp and *.bin
mkdir -p "$BENCHDIR"
...
log "=== R209 start (lock held; the daily stays UP on :8020) ==="
...
mix_row(){
  ...
  local out="$R/${arm}-${row}.txt" jf="/l2/_r209/${arm}-${row}.json"
  ...
  docker exec vllm-27b vllm bench serve \
    ...
    --result-filename "$jf" --save-result > "$out" 2>&1
  local rc=$? t1=$(date +%s.%N)
  mark "$arm" "$row" "$t0" "$t1"
  [ $rc -eq 0 ] || log "  WARN row $arm/$row exited $rc (see $out)"
  mv -f "$BENCHDIR/${arm}-${row}.json" "$R/" 2>/dev/null
...
arm_rows(){
  ...
  # mix-8k: decode-leaning. 8K prompts against 600-token answers => prefill is a minority of the work, so this is the regime a
  # few coding agents actually produce.
  mix_row "$arm" mix-8k  8000  600 48 '{"input":0.4,"output":0.6}' "$(( NONCE * 1000 + seedbase * 10 + 1 ))"
  # mix-32k: prefill-leaning. Deep prompts are the only workload R208 found above 400 W, so if the cap bites anywhere it is here.
  mix_row "$arm" mix-32k 32000 400 24 '{"input":0.3,"output":0.6}' "$(( NONCE * 1000 + seedbase * 10 + 2 ))"
  log "== ARM $arm done == tier=$(tier_gb) GB"
}

log "### R209 mixed prefill+decode, live daily on :8020 ###"
arm_rows A0 10
