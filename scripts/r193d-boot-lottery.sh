#!/usr/bin/env bash
# R193d (2026-09-05): is the per-BOOT numerics draw the runtime Triton autotune? R193d put four boots on ONE loaded artifact (OFF-h fresh-
# saved, BSS-h, OFF-h2, BSS-h2 loaded) and got three distinct ctx-0 outputs and four distinct 30K outputs, cutting across the BSS flag
# (BSS-h2 ≡ OFF-h at ctx 0, ≠ at 30K). So on top of the compile lottery (R193, bundled in the artifact) there is a per-process draw that
# the artifact does not pin. Candidates: @triton.autotune kernels benchmarked at first call — vllm/third_party/flash_linear_attention/ops
# (chunk_o, chunk_scaled_dot_kkt, solve_tril, chunk_delta_h, cumsum: the GDN chunked-prefill path, which is why 30K has more classes
# than ctx 0) and the mamba ssd_* ops. vLLM ships VLLM_TRITON_FORCE_FIRST_CONFIG=1 (triton_utils/force_first_config.py): every autotuned
# kernel takes its first config, no benchmarking. Arms P1, P2: BSS off, that knob on, both loading the r193b artifact (must be SAME-
# ARTIFACT). Gate: P2 vs P1 20/20 median 0 at ctx 0 AND 30K = the draw is the autotune and the knob pins it (then rulers must run with it
# on, and its speed cost — c8 x2, c16 x2, c1 x2 vs OFF-h2 — is the price of a deterministic ruler); P2 ≠ P1 = the draw is elsewhere
# (FlashInfer autotune cache, cuBLAS, the pcie_ipc tactic table). Own chain after r193c; restores the daily at the end.
#   unit: sudo systemd-run --unit=r193d-boot-lottery --collect -p User=adrienbrault -p RuntimeMaxSec=43200 -p TimeoutStopSec=900 \
#         -E GPU_QUEUE_NAME=r193d-boot-lottery bash -c '. /srv/qwen5090/lib/gpu-queue.sh; while systemctl is-active -q r193c-bss-determinism; do sleep 30; done; exec bash /srv/qwen5090/r193d-boot-lottery.sh'
set -uo pipefail
export PATH="$HOME/.local/bin:$PATH"
R=/srv/qwen5090/results/2026-09-05-r193d-boot-lottery; mkdir -p "$R"
RC=/srv/qwen5090/results/2026-09-05-r193c-bss-determinism
RB=/srv/qwen5090/results/2026-09-05-r193b-bss-artifact
for f in dec-OFF-h-ctx0 dec-OFF-h-ctx30000; do [ -f "$RB/$f.jsonl" ] || { echo "ABORT: $RB/$f.jsonl missing"; exit 3; }; done
...
finish(){ teardown; log "restoring daily (skipped if another unit is queued: $(gpu_queue_others | tr '\n' ' '))"; bash /srv/qwen5090/daily-restore-retry.sh 2>&1 | grep -aE "DAILY|FAILED|KV pool|attempt|SKIPPED" | cut -c1-160 | tee -a "$R/audit.log"; log "=== R193d $1 ==="; }
...
HAVE_LOCK=1
log "=== R193d start (lock held): BSS on the daily host artifact, $IMG (PCIE_IPC=1) — OFF-h, BSS-h ==="
...
arm P1 0 EXTRA_ENV_APPEND="-e VLLM_TRITON_FORCE_FIRST_CONFIG=1"
arm P2 0 EXTRA_ENV_APPEND="-e VLLM_TRITON_FORCE_FIRST_CONFIG=1"
...
h_b=$(grep -a '\[BSS-h\] BOOT OK' "$RB/audit.log" | grep -aoE 'compile_hashes=[0-9a-f,]*'); h_1=$(grep -a '\[P1\] BOOT OK' "$R/audit.log" | grep -aoE 'compile_hashes=[0-9a-f,]*'); h_2=$(grep -a '\[P2\] BOOT OK' "$R/audit.log" | grep -aoE 'compile_hashes=[0-9a-f,]*')
if grep -aq '\[P1\] BOOT OK.*SAME-ARTIFACT' "$R/audit.log" && grep -aq '\[P2\] BOOT OK.*SAME-ARTIFACT' "$R/audit.log" && [ -n "$h_b" ] && [ "$h_b" = "$h_1" ] && [ "$h_b" = "$h_2" ]; then log "SAME-ARTIFACT: P1, P2 loaded the r193b artifact ($h_b)"
else log "CONFOUNDED: an arm fresh-compiled or the hash sets differ (r193b $h_b / P1 $h_1 / P2 $h_2)"; fi
cmp(){ log "[$1 vs $2 decode ctx$3] $(python3 $PR/decode_fidelity.py compare "$4" "$5" 2>&1 | tail -1 | cut -c1-300)"; }
for ctx in 0 30000; do
  [ -f "$R/dec-P1-ctx$ctx.jsonl" ] && [ -f "$R/dec-P2-ctx$ctx.jsonl" ] && cmp P2 P1 $ctx "$R/dec-P1-ctx$ctx.jsonl" "$R/dec-P2-ctx$ctx.jsonl"
  [ -f "$R/dec-P1-ctx$ctx.jsonl" ] && cmp P1 OFF-h $ctx "$RB/dec-OFF-h-ctx$ctx.jsonl" "$R/dec-P1-ctx$ctx.jsonl"
  [ -f "$R/dec-P1-ctx$ctx.jsonl" ] && [ -f "$RC/dec-OFF-h2-ctx$ctx.jsonl" ] && cmp P1 OFF-h2 $ctx "$RC/dec-OFF-h2-ctx$ctx.jsonl" "$R/dec-P1-ctx$ctx.jsonl"
done
grep -ac 'force_first_config\|FORCE_FIRST' "$R/engine-boot-P1.log" | sed 's/^/[P1 force-first-config log lines] /' | tee -a "$R/audit.log"
