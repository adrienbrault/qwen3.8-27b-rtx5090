...
set -uo pipefail
TAG=$1; R=$2; U=${U:-http://127.0.0.1:8030}; EVICT=${EVICT:-12}; EVICT_CTX=${EVICT_CTX:-90000}; EVICT_REASKS=${EVICT_REASKS:-2}; EVICT_REASK_GAP=${EVICT_REASK_GAP:-15}
python3 /srv/qwen5090/probes/needle_depth.py --url $U/v1 --model qwen3.8-27b --depths 131000 220000 --samples 2 \
  --evict "$EVICT" --evict-ctx "$EVICT_CTX" --evict-reasks "$EVICT_REASKS" --evict-reask-gap "$EVICT_REASK_GAP" --out "$R/needles-$TAG.jsonl" > "$R/needles-$TAG.out" 2>&1; rc=$?
