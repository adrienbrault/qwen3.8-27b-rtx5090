...
set -uo pipefail
TAG=$1; R=$2; U=${U:-http://127.0.0.1:8030}; EVICT=${EVICT:-12}; EVICT_CTX=${EVICT_CTX:-90000}
python3 /srv/qwen5090/probes/needle_depth.py --url $U/v1 --model qwen3.8-27b --depths 131000 220000 --samples 2 \
  --evict "$EVICT" --evict-ctx "$EVICT_CTX" --out "$R/needles-$TAG.jsonl" > "$R/needles-$TAG.out" 2>&1; rc=$?
