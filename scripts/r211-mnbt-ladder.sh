#!/usr/bin/env bash
# R211 — the prefill CHUNK ladder under agent-shaped load, on :8029.
#
...
#
#   unit: sudo systemd-run --unit=r211-mnbt-ladder --collect -p User=adrienbrault -p RuntimeMaxSec=43200 -p TimeoutStopSec=300 \
#         -E GPU_QUEUE_NAME=r211-mnbt-ladder bash -c '. /srv/qwen5090/lib/gpu-queue.sh; exec bash /srv/qwen5090/r211-mnbt-ladder.sh'
set -uo pipefail
export PATH="$HOME/.local/bin:$PATH"
R=/srv/qwen5090/results/2026-09-08-r211-mnbt-ladder; mkdir -p "$R"
...
log "=== R211 start (lock held; the daily goes DOWN for the ladder) ==="
...
log "### R211 chunk ladder: A(8192) B(4096) C(1536) D(16384) E(4096,SEQS32) A2(8192 repeat) ###"
run_arm A  8192 16
run_arm B  4096 16
run_arm C  1536 16
run_arm D 16384 16
...
log "### R211 rows done — restoring the daily ###"
restore_daily
log "### R211 DONE ### results in $R"
