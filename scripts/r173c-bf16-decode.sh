...
# bf16 = 52 GB of 64 GB (R156 arm a): MAXLEN 32768 at util 0.92, SEQS 1; falls back to MAXLEN 3072 (ctx0 only) if the boot fails.
...
finish(){ teardown; log "restoring daily (skipped if another unit is queued: $(gpu_queue_others | tr '\n' ' '))"; bash /srv/qwen5090/daily-restore-retry.sh 2>&1 | grep -aE "DAILY|FAILED|KV pool|attempt|SKIPPED" | cut -c1-160 | tee -a "$R/audit.log"; log "=== R173c $1 ==="; }
...
MAXLEN=32768
if boot_bf16 32768; then :; else
  log "bf16 boot at MAXLEN 32768 FAILED: $(grep -aE 'FAILED|Error' "$R/boot-bf16-32768.log" | tail -1 | cut -c1-200); retrying at 3072 (ctx0 only)"; teardown
...
dfid 0; [ "$MAXLEN" -ge 32768 ] && dfid 30000
...
for c in 0 30000; do [ -f "$R/dec-bf16-ctx$c.jsonl" ] || continue
