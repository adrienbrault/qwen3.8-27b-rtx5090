#!/usr/bin/env bash
# R209b deep mixed prefill+decode -- control-clean re-run of R209's disqualified mix-32k row (2026-09-07, user "have you measured perf impact when many requests are prefilling/decoding?
...
#   unit: sudo systemd-run --unit=r209b-mixed-deep --collect -p User=adrienbrault -p RuntimeMaxSec=21600 -p TimeoutStopSec=300 \
#         -p 'ExecStopPost=-/bin/bash /srv/qwen5090/r208-restore-power.sh' \
#         -E GPU_QUEUE_NAME=r209b-mixed-deep bash -c '. /srv/qwen5090/lib/gpu-queue.sh; exec bash /srv/qwen5090/r209b-mixed-deep.sh'
set -uo pipefail
export PATH="$HOME/.local/bin:$PATH"
R=/srv/qwen5090/results/2026-09-07-r209b-mixed-deep; mkdir -p "$R"
...
log "=== R209b start (lock held; the daily stays UP on :8020) ==="
...
mix_row(){
  ...
  docker exec vllm-27b vllm bench serve \
    ...
    --request-rate 0.6 --seed "$seed" --ignore-eos --percentile-metrics ttft,tpot,itl,e2el \
    > "$out" 2>&1
...
arm_rows(){
  ...
  mix_row "$arm" mix-32k-deep 32000 800 32 '{"input":0,"output":0.5}' "$(( NONCE * 1000 + seedbase * 10 + 1 ))"
  log "== ARM $arm done == tier=$(tier_gb) GB"
}

log "### R209b deep mixed prefill+decode (control-clean re-run of R209 mix-32k), live daily on :8020 ###"
arm_rows A0 10
