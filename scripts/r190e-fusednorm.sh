...
#   unit (re-issued 2026-09-05 07:25 UTC after the 2nd FN boot failure — op not registered on a cached-artifact boot, fixed in 0144, image rebuilt; chain r190d → r193 → r190e → r189): sudo systemd-run --unit=r190e-fusednorm --collect -p User=adrienbrault -p RuntimeMaxSec=43200 -p TimeoutStopSec=900 \
#         -E GPU_QUEUE_NAME=r190e-fusednorm bash -c '. /srv/qwen5090/lib/gpu-queue.sh; while systemctl is-active -q r193-determinism; do sleep 30; done; exec bash /srv/qwen5090/r190e-fusednorm.sh'
